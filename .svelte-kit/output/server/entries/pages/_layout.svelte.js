...
import { V as escape_html, a as head, et as getContext, i as ensure_array_like, l as unsubscribe_stores, n as attr_class, s as store_get, z as attr } from "../../chunks/dev.js";
	...
	$$renderer.component(($$renderer) => {
		...
		head("12qhfyh", $$renderer, ($$renderer) => {
			$$renderer.title(($$renderer) => {
				$$renderer.push(`<title>真菌观察记录</title>`);
			});
		...
		$$renderer.push(`<div class="min-h-screen bg-surface-50-900-token"><header class="bg-primary-500-900-token text-white shadow-lg"><div class="container mx-auto px-4 py-4"><div class="flex items-center justify-between"><div class="flex items-center gap-3"><span class="material-icons text-3xl">nature</span> <h1 class="text-2xl font-bold">真菌观察记录</h1></div></div></div></header> <nav class="bg-surface-100-800-token shadow"><div class="container mx-auto px-4"><div class="flex gap-1 py-2"><!--[-->`);
		const each_array = ensure_array_like(navItems);
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			let item = each_array[$$index];
			$$renderer.push(`<a${attr("href", item.path)}${attr_class(`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${store_get($$store_subs ??= {}, "$page", page).url.pathname === item.path ? "bg-primary-500 text-white" : "hover:bg-surface-200-700-token text-surface-700-200-token"}`)}><span class="material-icons text-xl">${escape_html(item.icon)}</span> <span>${escape_html(item.label)}</span></a>`);
		}
